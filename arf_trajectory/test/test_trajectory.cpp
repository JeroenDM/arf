...
TEST(TestNumber, constructor)
{
  Number x(3.0);
  Number y;
}

TEST(TestNumber, casting)
{
  Number x(3.0);
  double y = x;
  EXPECT_EQ(y, 3.0);
...
TEST(TestTolerancedNumber, constructor)
{
  TolerancedNumber x(0.5, -1, 2);
  TolerancedNumber y(0.5, -1, 2, 6);
}

TEST(TestTolerancedNumber, wrongInput)
{
  try
  {
...
TEST(TestTrajectoryPoint, constructor)
{
...
TEST(TestTrajectoryPoint, gridSampling)
{
  TolerancedNumber x(1, 1, 2, 2);
  Number y(5);
  TolerancedNumber z(1, 1, 3, 3);
